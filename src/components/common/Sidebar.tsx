...
const Sidebar = (props:MENU_LIST) =>{
  ...
  // 초기 로드 시 설정
  let defaulSize = (2133 - window.innerWidth)+ -1735;
  const ResizeEnvet = () =>{
    let numPx = transitionPoint();
    defaulSize = (2133 - window.innerWidth)+ -1735;
    if(numPx != 0){
      ref2e.current.style.transform = "translateX("+defaulSize+"px)";
    }
  }
  ...
  return (
  <div className='sidebar sideT' style={{transform:"translateX("+defaulSize+"px)"}} onMouseMove={(e) => moveListener(e)} onMouseDown={(e)=> mouseDown(e)} ref={ref2e}>
